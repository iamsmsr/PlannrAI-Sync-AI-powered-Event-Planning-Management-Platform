...
class VectorStore:
    ...
    # def load_from_mongodb(self, mongo_uri, db_name="event_managment", collection_name="vector"):
    #     from pymongo import MongoClient
    #     client = MongoClient(mongo_uri)
    #     db = client[db_name]
    #     collection = db[collection_name]
    #     doc = collection.find_one()
    #     if doc and "vectors" in doc:
    #         self.store = doc["vectors"]
    #     else:
    #         self.store = []
    ...
    def load_from_mongodb(self, mongo_uri, db_name="event_managment", collection_name="vector"):
        ...
        db = client[db_name]
        collection = db[collection_name]
        doc = collection.find_one({"type": "vector_store_json"})
        if doc and "vectors" in doc:
            self.store = doc["vectors"]
        else:
            self.store = []
